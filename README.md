...
- Website **Satcom / Inmarsat / Aircraft / SDRplay** panels are in this **1.1.38 source tree** (not GitHub Latest 1.1.33). They need SDR Town **0.2.67–0.2.74** and the matching 0.2.74 `SdrTownControl.dll`. Satcom start from the site does not send Town `force=true` (listen session may refuse). Inmarsat in Town 0.2.74 is a **prototype** (no unique-word/FEC). Observer home on the FUBAR site is lat/lon text, not Town’s clickable OSM map. Full matrix: SDR Town `docs/FUBAR_PAIRING.md`.
