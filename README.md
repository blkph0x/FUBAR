...
capture PCM (1:1). The page detects the browser and uses a hidden hold element matched
to that device (video on iPhone, audio on Android Chrome) so the stream can stay up when
the app is backgrounded. A screen wake lock is still taken while the page is in front. Set
